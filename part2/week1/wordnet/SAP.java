...
public class SAP {
    ...
    private int bfs(Queue<Integer> qV, Queue<Integer> qW, boolean distance) {
        ...
        int distSoFarV = 1, distSoFarW = 1;
        while ((!qV.isEmpty() || !qW.isEmpty())) {
            if ((distSoFarV > shortestLen) || (distSoFarW > shortestLen)) break; // early terminate
            while (!qV.isEmpty()) {
                if (distSoFarV == distV[qV.peek()]) break; // only search same level

                int pioneer = qV.dequeue();
                for (int adjV : digraph.adj(pioneer)) {
                    if (markedW[adjV]) {
                        int tmpLen = distW[adjV] + distSoFarV;
            ...
            while (!qW.isEmpty()) {
                if (distSoFarW == distW[qW.peek()]) break; // only search same level

                int pioneer = qW.dequeue();
                for (int adjW : digraph.adj(pioneer)) {
                    if (markedV[adjW]) {
                        int tmpLen = distV[adjW] + distSoFarW;
                        if (tmpLen < shortestLen) {
                            ancestor = adjW;
                            shortestLen = tmpLen;
                        }
